...
```{r, read_data}
data_url <- 'https://raw.githubusercontent.com/jyurko/CS_1675_Spring_2022/main/HW/08/hw08_data.csv'
...
The relationship between x1 and y seems to be a little unclear at the moment, but with x2 and y there appears to be a quadratic realationship. 
...
No, none of the features of statistically significant. All confidence intervals contain 0, meaning that it is plausible for that particular coefficient to be 0. 
...
With the interaction term present, there are 4 features (intercept, x1, x2, x1*x2)
Now, x1 and the interaction term x1*x2 are considered to be statistically significant because their confidence intervals do not contian 0. 
...
There are 9 features (intercept + x1,x2, x1^2, x2^2, x1:x2, x1:x2^2, x1^2: x2, x1^2:x2^2)
The intercept and x2^2 features are statistically significant. 
...
viz_post_coefs(laplace_03_strong$mode[-length(laplace_03_strong$mode)],sd, colnames(X03))
```
...
viz_post_coefs(laplace_06_strong$mode[-length(laplace_06_strong$mode)],sd, colnames(X06))
```
...
sd <- sd[-length(sd)]
viz_post_coefs(laplace_03_very_strong$mode[-length(laplace_03_very_strong$mode)],sd, colnames(X03))
```
...
sd <- sd[-length(sd)]
viz_post_coefs(laplace_06_very_strong$mode[-length(laplace_06_very_strong$mode)],sd, colnames(X06))
```
