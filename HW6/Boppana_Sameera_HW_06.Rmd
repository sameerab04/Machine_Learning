...
The sample average is slightly less than the poster MAP but is pretty similar. The posterior mode appears to be slightly above 10.5 while the sample average appears to be roughly 10.25. 
The sample standard deviation appears to be slightly higher than the standard deviation on the posterior mode, meaning that the uncertainty in the posterior is slightly less than the sample uncertainty. 
...
$$
\frac{d}{d_\mu}(\log[ \prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \times \mathrm{normal}\left( \mu \mid \mu_0, \tau_0\right)]) =  \frac{N}{\sigma^2}(\bar{x} - \mu) - \frac{1}{\tau_0^2}(\mu - \mu_{0})

$$
$$
\frac{N}{\sigma^2}(\frac{d}{d_\mu}(\bar{x}-\mu)) - \frac{1}{\tau_0^2}(\frac{d}{d_\mu}(\mu)- \frac{d}{d_\mu}(\mu_0)) \\
\frac{N}{\sigma^2}(\frac{d}{d_\mu}(\bar{x}) - \frac{d}{d_\mu}(\mu))- \frac{1}{\tau_0^2} = \\
\frac{d^"}{d_\mu}(\log[ \prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \times \mathrm{normal}\left( \mu \mid \mu_0, \tau_0\right)]) =  \frac{-N}{\sigma^2} - \frac{1}{\tau_0^2}
$$
...
What do you think? 
You can confirm that the mode does correspond to the $\mu$ value associated with the maximum log-posterior since we know the second derivative we can calculate the sign of the second derivative at the mode/$\mu$. The mode is when the second derivative is equal to zero as this represents the "top of the hill". If the points less than the mode are positive and points slightly greater than the mode are negative, this indicates a maximum.  
...
$$
\mathrm{m}_N= \frac{\frac{\mathrm{m}_0}{\mathrm{s}_0^2} + \frac{N\bar{x}}{\sigma^2}}{\frac{1}{\mathrm{s}_0^2} + \frac{N}{\sigma^2}}
$$
$$
\frac{1}{\mathrm{s}_N^2} = \frac{1}{\tau_0^2} + \frac{N}{\sigma^2}
$$
...
What do you think?  
THe Laplace Approximation in this specific application is equal to the exact posterior distribution because the the posterior is a normal distribution, so with enough observations, the approximate distribution with converge to the true distribution. 
...
What do you think?  
Sigma is bounded, so when we transform $\sigma$ to $\varphi$ we get a new unbounded variable. This is helpful for the Laplace Approximation because now both $\mu$ and $\varphi$ are unbounded instead of using one unbounded ($\mu$) and one bounded ($\sigma$).
...
$$
p(\mu, \varphi |\textbf{x}) = \prod_{n-1}^N (p (x_n | \mu, g^{-1}(\varphi))) \times p(\mu|\mu_0,\tau_0)\times p(g^{-1}(\varphi)|l,u)\times \textbf{|}\frac{d}{d_\varphi}(g^{-1}(\varphi))\textbf{|} 
$$
...
$$
\varphi = log(\sigma) = g(\sigma) \\
\sigma = \exp^\varphi
$$

$$
\frac{d}{d_\sigma}(exp^\varphi) = exp^\varphi\\
...
```
The posterior mean for $\mu$ is 10.85589 and the posterior mean for $\varphi$ is 1.09389

```{r}
laplace_result$var_matrix
```
The posterior correlation coefficient is 1.15 between $\mu$ and $\varphi$
...
Include as many code chunks and discussion text as you feel are necessary.  
Yes, the result of the inverse link function to to posterior mean on varphi is the same as the posterior mean on sigma. 

### 5d)

**Use `ggplot2` to visualize the posterior histograms on the $\mu$ and $\sigma$ parameters. Sejht the number of bins to 55. You may use separate `ggplot()` calls for each histogram.**  
...
```
The posterior probability that $\sigma$ is greater than 4 is 0.1804.
