...
```{r, solution_01a}
ggplot(data = train_01) + geom_point(mapping = aes(x = x, y = y)) + geom_smooth(mapping = aes(x, y), method = 'lm')
```
...
all_spline_mae_results %>%  
  ggplot(mapping = aes(x = as.factor(J), y = mae)) + geom_boxplot(mapping = aes(x = as.factor(J), fill = all_spline_mae_results$dataset)) + 
...
tibble::tibble(
  ...
  mutate(J = spline_dof) %>% 
  ggplot(x = as.factor(J), y = w) + geom_bar(mapping = aes( x = as.factor(J), y = w), stat = "identity")
```
